...
RFsize = 100;                    % the average actual size of conv5, you could change it to get a tighter segmentation
para.gridScale = [13 13];       % conv5 of alexNet feature map
para.imageScale = [256 256];    % the input image size
...
for i=1:20
    ...
    subplot(1,20,i),imshow(curSegmentation);
    savefilename = ['pictures/' imagenames{i}];
    imwrite(curSegmentation,savefilename);
end
